...
class Post(models.Model):
    ...
    # будет делаться к одному посту, и каждый пост может содержать несколько комментариев.
    # Атрибут related_name позволяет назначать имя атрибуту, который используется
    # для связи от ассоциированного объекта назад к нему.
    # Пост комментарного объекта можно извлекать посредством comment.post
    # и все комментарии, ассоциированные с объектом-постом, – посредством post.comments.all().
    # Если атрибут related_name не определен, то Django будет использовать
...
class Comment(models.Model):
    ...
    # ментария. Используя auto_now_add, дата будет сохраняться автоматически при создании объекта.
    created = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)

    # Мы определили булево поле active, чтобы управлять статусом комментариев.
    # Данное поле позволит деактивировать неуместные комментарии вручную с помощью
    # сайта администрирования. Мы используем параметр default=True,
    # чтобы указать, что по умолчанию все комментарии активны.
    active = models.BooleanField(default=True)

    # В Meta-класс модели был добавлен атрибут ordering = ['created'], чтобы по умолчанию сортировать комментарии
